...
/**
 * Data structure for LLM interaction auditing.
 * Contains the input prompt sent to the LLM and the output response received.
 *
 * @example
 * ```typescript
 * const interactionData: LlmInteractionData = {
 *   input: "<system>You are a helpful assistant</system>",
 *   output: '{"thought": "I should help the user", "response_to_user": "Hello!"}'
 * };
 * ```
 */
export type LlmInteractionData = {
  /** The prompt/input text sent to the LLM */
  input: string;
  /** The response text received from the LLM */
  output: string;
};

/**
 * Callback function type for tracking LLM interactions for auditing purposes.
 * Called after each complete LLM interaction to log input/output pairs.
 *
 * @param data - Object containing the input prompt and output response
 *
 * @example
 * ```typescript
 * const auditHandler: LlmInteractionCallback = (data) => {
 *   console.log('LLM Input:', data.input);
 *   console.log('LLM Output:', data.output);
 *   // Store in audit log, database, etc.
 * };
 * ```
 */
export type LlmInteractionCallback = (data: LlmInteractionData) => void;

/**
 * Function type for calling an LLM (Large Language Model).
 * Handles the communication with the underlying AI model.
 *
 * @param input - The prompt/input text to send to the LLM. This is the complete conversation
 *                context including system messages, conversation history, and the current user query.
 * @param onStreamingChunk - Optional callback function that receives text chunks as they are
 *                           generated by the LLM during streaming responses. Each chunk contains
 *                           a partial response that can be displayed to the user in real-time.
 * @param onLlmInteraction - Optional callback function for audit logging and tracking purposes.
 *                           Called once after the LLM completes its response with the full input
 *                           and output data for compliance and debugging purposes.
 * @returns Promise that resolves to the LLM response containing the generated content and usage
 *          metadata, or undefined if the implementation only supports streaming without returning
 *          the complete response.
 *
 * @example
 * ```typescript
 * const geminiLlm: CallLlm = async (input, onStreaming, onInteraction) => {
